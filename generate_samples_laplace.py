...
class SyntheticDAG:
    def __init__(self, nnodes, DAG_type, f=lambda x: torch.norm(x, p=2)**2, seed=1234):
        ...
        self.locations = [0 for _ in range(nnodes)] #location parameters of laplace
        self.scales = [1 for _ in range(nnodes)] #scale parameters of laplace
    ...
    def sample_scaled(self, G, num_samples=500):
        ...
        for i in self.top_order:
            scale = random()
            noise = np.random.laplace(loc=0.0, scale=scale, size=num_samples)
    ...
    def remove_sinks(self):
        ...
        for sink in sorted_sinks:
            ...
            # print(name_map)
            self.nnodes-=1
            self.scales.pop(sink) #torch.cat((self.variances[:sink], self.variances[sink+1:]))
            self.locations.pop(sink) #= torch.cat((self.means[:sink], self.means[sink+1:]))
