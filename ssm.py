    ...
    def fit(
        ...
        for epoch in range(epochs):
            ...
            if (epoch + 1) % eval_step == 0 or (epoch + 1) == epochs:
                ...
                # logging.info(
                #     f"Epoch:{epoch + 1:6}/{epochs:6}, val_loss={val_loss:+.10f}"
                #     + (
                #         f", train_loss={train_loss:+.10f}"  # type: ignore
                #         if self.args.print_train_losses
                #         else ""
                #     )
                # )
...
def ssm_hess(X_obs, X_tr, score_est_steps=50):
    score_estimator = score_fn_from_data(X_tr.unsqueeze(2), epochs=score_est_steps)
    jacobian_estimator = jacrev(score_estimator)
    jx_obs = torch.zeros(X_obs.shape[0], X_obs.shape[1], X_obs.shape[1])
    for i in range(len(X_obs)):
        jx_obs[i] = jacobian_estimator(X_obs[i])
    return jx_obs
