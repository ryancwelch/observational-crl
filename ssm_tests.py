...
def append_value_to_csv(value, num_samples):
    with open(f'tests/ssm/{num_samples}.csv', mode='a', newline='') as file:
...
# U, X, noises = graph.sample_scaled(G, 50000)
# id_layers = identify_e(X, estimator='stein')
# noise_estimates = noise_estimation(id_layers, layers)
# append_value_to_csv(mac(noises, noise_estimates))

num_samples_list = [10000, 100000, 1000000, 10000000]
num_trials = 10
...
for num_samples in num_samples_list:
    for i in range(num_trials):
        ...
        num_select = min(num_samples, 6000)
        indices = torch.randperm(num_samples-4)[:num_select]
        U = U_tr[indices]
        X = X_tr[indices]
